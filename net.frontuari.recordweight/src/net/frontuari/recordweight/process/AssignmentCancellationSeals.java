...
public class AssignmentCancellationSeals extends CustomProcess {
	...
	protected String doIt() throws Exception {
		...
		if (p_IsVoided) {
			String sql = "SELECT il.M_InventoryLine_ID " + "FROM M_Inventory i "
					...
					+ "WHERE i.FTU_LoadOrder_ID = ? AND il.M_Product_ID = ? AND ilma.M_AttributeSetInstance_ID = ?";
			int p_M_InventoryLine_ID = DB.getSQLValue(get_TrxName(), sql, p_FTU_LoadOrder_ID, p_M_Product_ID,
			...
			inventory.set_ValueOfColumn("FTU_LoadOrder_ID", p_FTU_LoadOrder_ID);
			inventory.saveEx();
	...
	private MStorageOnHand[] getWarehouse(Properties ctx, int M_Warehouse_ID, int M_Product_ID,
		...
		sql += " AND NOT EXISTS ( " + "SELECT 1 " + "FROM M_AttributeSetInstance "
				...
				+ "	AND M_InventoryLine.M_Product_ID = s.M_Product_ID" + "	AND M_Inventory.FTU_LoadOrder_ID = "
				+ p_FTU_LoadOrder_ID + ")";
